...
개발자 또는 테스터가 `Jump Tuning` 창에서 주인공 물리 크기, 시각 크기 배율, 시각 X/Y 오프셋을 조정한다.
...
- 사용자가 `Player Body > Player Square Size`, `Player Visual Scale`, `Player Visual X Offset`, `Player Visual Y Offset` 값을 변경한다.
...
8. 사용자는 필요하면 주인공 시각 요소의 X/Y 오프셋을 입력한다.
9. 시스템은 X/Y 오프셋만큼 보이는 이미지를 좌우 또는 위아래로 이동하고 `BoxCollider2D` 크기와 위치는 유지한다.
10. 시스템은 좌우반전 시 `BoxCollider2D` 윗변의 중점을 지나는 세로선을 기준으로 X 오프셋을 반대편으로 반전한다.
11. 시스템은 지면 판정이 변경된 충돌체 하단과 폭을 기준으로 동작하게 한다.
...
- [ ] `Jump Tuning` 창에서 주인공 시각 X 오프셋을 변경할 수 있다.
- [ ] `Jump Tuning` 창에서 주인공 시각 Y 오프셋을 변경할 수 있다.
- [ ] 물리 크기 변경 시 `BoxCollider2D` 크기가 변경된다.
- [ ] 시각 크기 배율 변경 시 스프라이트 크기만 변경되고 `BoxCollider2D` 크기는 유지된다.
- [ ] 시각 X/Y 오프셋 변경 시 스프라이트 위치만 변경되고 `BoxCollider2D` 크기와 위치는 유지된다.
- [ ] 시각 크기 배율을 키워도 Player 스프라이트 하단은 충돌체 하단 기준에 맞춰지고 위쪽으로 커진다.
- [ ] 좌우반전된 Player 스프라이트는 `BoxCollider2D` 윗변의 중점을 지나는 세로선을 기준으로 반전된다.
...
- `JumpTuningConfig.PlayerVisualXOffset`이 플레이어 시각 X 오프셋 설정을 보관한다.
- `JumpTuningConfig.PlayerVisualYOffset`이 플레이어 시각 Y 오프셋 설정을 보관한다.
- `Jump Tuning` 창의 `Player Body` 섹션에서 `Player Square Size`, `Player Visual Scale`, `Player Visual X Offset`, `Player Visual Y Offset`을 함께 수정할 수 있다.
...
- `PlayerVisual`은 하단 정렬 이후 `Player Visual X Offset`과 `Player Visual Y Offset` 값을 더해 캐릭터 이미지만 좌우 또는 위아래로 이동한다.
- `PlayerVisual`은 좌우반전 시 `Player Visual X Offset`을 `BoxCollider2D` 윗변 중점의 세로선 반대편으로 반전해 충돌체 기준 반전축을 유지한다.
