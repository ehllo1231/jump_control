...
3. 사용자가 생성 버튼을 누른다.
4. 시스템은 선택한 도형이 삼각형이면 자식 시각 오브젝트의 삼각형 메시와 부모 `PolygonCollider2D`를 가진 Platform을 생성한다.
5. 시스템은 삼각형 시각 메시와 `PolygonCollider2D`가 같은 꼭지점 좌표를 사용하도록 갱신한다.
6. 사용자는 Scene View에서 삼각형 꼭지점을 드래그해 원하는 삼각형 모양으로 변경한다.
7. 시스템은 꼭지점 변경 결과를 Platform에 저장하고 Width, Height를 삼각형 외곽 크기에 맞춰 갱신한다.
8. 시스템은 선택한 도형이 사각형이면 기존처럼 SpriteRenderer와 `BoxCollider2D`를 가진 Platform을 생성한다.
9. 생성된 Platform은 Scene View에서 이동할 수 있고 Inspector에서 도형, Width, Height와 삼각형 꼭지점을 수정할 수 있다.
10. 사용자가 씬을 저장하고 Unity를 다시 열거나 게임을 실행해도 생성한 삼각형 Platform이 사라지지 않는다.
...
- 4b. 부모 Platform에 `SpriteRenderer`가 있어도 시스템은 삼각형 메시 렌더러를 별도 자식 오브젝트에 생성해 Unity Renderer 컴포넌트 충돌을 피한다.
- 6a. 사용자가 삼각형을 거의 일직선이나 0 크기에 가깝게 만들면 시스템은 유효한 삼각형 면적이 유지되는 범위에서만 꼭지점 변경을 적용한다.
...
- 사각형 Platform은 기존처럼 SpriteRenderer와 `BoxCollider2D`를 사용한다.
- 삼각형 Platform은 세 꼭지점 좌표를 직렬화하고, 자식 시각 오브젝트의 MeshRenderer/MeshFilter와 부모 `PolygonCollider2D`가 같은 꼭지점 배열을 사용한다.
- Unity는 같은 GameObject에 `SpriteRenderer`와 `MeshRenderer`를 함께 둘 수 없으므로, 삼각형 MeshRenderer/MeshFilter는 `Triangle Visual` 자식 오브젝트에 생성한다.
