...
9. 시스템은 시각 Y 오프셋을 적용해 캐릭터 이미지만 위 또는 아래로 이동할 수 있게 한다.
10. 시스템은 캐릭터 이미지 스프라이트를 표시하더라도 원본 색상을 유지할 수 있게 한다.
...
- 9a. 시각 Y 오프셋이 음수이면 시스템은 충돌체를 움직이지 않고 캐릭터 이미지만 아래로 내린다.
- 10a. 상태별 색상 틴트가 필요한 임시 사각형 스프라이트를 사용할 경우 시스템은 기존처럼 상태 색상을 적용할 수 있다.
...
- [ ] Player 시각 Y 오프셋을 변경해도 `BoxCollider2D` 크기와 위치는 유지된다.
...
- `PlayerVisual`은 하단 정렬 이후 시각 Y 오프셋을 더해 캐릭터 이미지만 위 또는 아래로 조정한다.
