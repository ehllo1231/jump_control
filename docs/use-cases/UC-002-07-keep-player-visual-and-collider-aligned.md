...
4. 시스템은 시각 요소의 기본 로컬 위치를 현재 크기, 오프셋, 방향에 맞는 정렬 위치로 되돌린다.
...
12. Play Mode에서는 기본 정렬 결과에 화면 픽셀 단위의 시각 보정만 추가하며 Player 루트와 충돌체는 변경하지 않는다.
...
- 12a. 픽셀 보정용 Orthographic Camera가 없거나 시각 픽셀 스냅이 비활성화되면 시스템은 기본 정렬 위치를 그대로 사용한다.

## 인수 조건

- [ ] Scene View에서 보이는 Player 시각 요소를 움직여도 충돌체가 이전 위치에 남지 않는다.
- [ ] Scene View에서 Player 드래그 핸들을 끌어 Player 위치를 자유롭게 수정할 수 있다.
- [ ] Player 시각 요소의 기본 로컬 위치는 현재 크기, 오프셋, 방향에 맞게 유지되고 Play Mode 픽셀 보정은 시각 요소에만 적용된다.
...
- [ ] 런타임 픽셀 위치 보정은 Player 루트와 `BoxCollider2D`의 위치 및 크기를 변경하지 않는다.
...
- 플레이어 시각 요소의 런타임 픽셀 보정 요구사항과 검증 결과는 `UC-011-02`에서 관리한다.
- Unity 6000.3.1f1 자동 검증에서 런타임 픽셀 보정 전후 Player 루트, Rigidbody2D 위치, `BoxCollider2D.offset/size`, 시각 스케일과 Z 값이 유지되는 것을 확인했다.
