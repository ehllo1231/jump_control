...
맵 제작자가 사각형 벽돌과 같은 Platform/Visual 구조로 직각 삼각형 벽돌을 생성하고 Scene View에서 위치와 크기를 조정한다.
...
3. 사용자가 생성 버튼을 누른다.
4. 시스템은 선택한 직각 삼각형 방향에 맞춰 Platform 루트에 `PolygonCollider2D`, `Visual` 자식에 `SpriteRenderer`를 가진 Platform을 생성한다.
...
9. 시스템은 편집 후에도 삼각형의 직각 구조를 유지하고 `Visual`의 `SpriteRenderer`, 루트의 `PolygonCollider2D`, Inspector Width와 Height를 동기화한다.
...
- [ ] 직각 삼각형 Platform은 루트 Platform과 직접 자식 `Visual`로 구성되며 별도 `Triangle Visual`은 만들지 않는다.
- [ ] 직각 삼각형 Platform은 선택한 방향에 맞는 직각 모서리와 빗변을 가진다.
- [ ] 직각 삼각형 Platform은 사각 Platform과 같은 색상 기준으로 표시된다.
- [ ] 직각 삼각형 Platform은 `Visual`의 `SpriteRenderer` 외형과 같은 꼭지점 좌표를 쓰는 루트 `PolygonCollider2D`로 충돌을 제공한다.
...
- [ ] 편집 후 `Visual`의 `SpriteRenderer`, 루트의 `PolygonCollider2D`, Inspector Width와 Height가 같은 외곽 크기를 반영한다.
...
- 직각 삼각형은 `Triangle Visual` 자식을 생성하지 않고 `Visual`의 SpriteRenderer에 절차 생성한 직각 삼각형 스프라이트를 적용한다.
- 직각 삼각형으로 전환된 기존 오브젝트에 `Triangle Visual` 자식이 남아 있으면 `Platform2D`가 해당 자식을 제거한다.
- 직각 삼각형의 `Visual` SpriteRenderer.size 변경을 감지해 Width, Height와 루트 `PolygonCollider2D` 경로를 동기화한다.
...
- 2026-07-17 SpriteRenderer를 `Visual` 자식으로 이전한 구조에서도 직각 삼각형 외형과 루트 PolygonCollider2D 동기화가 유지되도록 수정했다.
